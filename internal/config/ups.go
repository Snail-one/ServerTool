...
func applyNUTStandaloneMode(content string) string {
	...
	return appendBlock(content, "MODE=standalone\n")
}
...
func applyUPSDListenConfig(content string) string {
	if containsLine(content, upsListenLine) {
		return content
	}
	return appendBlock(content, upsListenLine+"\n")
}
...
func replaceNUTSection(content, sectionName, block string) string {
	...
	return appendBlock(cleaned, strings.TrimRight(block, "\n")+"\n")
}
