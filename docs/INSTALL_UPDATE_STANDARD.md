...
输出按“工具标题、版本检查、发布信息、执行步骤、最终结果”的顺序组织。标准输出示例。

连接到交互式终端且 `TERM` 不是 `dumb` 时，标题、步骤和信息标签使用 ANSI 颜色，其中橙色为标题、步骤及发布信息标签的主色；管道或重定向输出默认不带颜色。设置 `NO_COLOR` 可显式关闭颜色；需要让支持 ANSI 的日志采集器保留颜色时，可设置 `FORCE_COLOR=1`。
...
[步骤] 检查发布版本
[信息] 最新正式版本：v1.2.0（GitHub Releases API）
...
[步骤] 下载发布文件
[信息] 正在下载：example_linux_amd64_v1.2.0
############################ 100.0%
[结果] 发布文件下载完成
...
[步骤] 安装程序
[信息] 正在写入：/usr/local/sbin/example
[结果] 更新完成：/usr/local/sbin/example
[信息] 当前版本：v1.2.0
```
